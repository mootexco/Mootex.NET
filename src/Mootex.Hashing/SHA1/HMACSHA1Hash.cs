...
namespace Mootex.Hashing.SHA1 {
    ...
    public sealed class HMACSHA1Hash {

        public static byte[] CalculateBytes(Stream src, byte[] key) {
            return GenericHash.CalculateBytes(src, () => new HMACSHA1(key));
        }

        public static byte[] CalculateBytes(Stream src, string key) {
            return GenericHash.CalculateBytes(src, () => new HMACSHA1(Encoding.UTF8.GetBytes(key), true));
        }
        ...
        public static string CalculateString(Stream src, byte[] key) {
            return GenericHash.CalculateString(src, () => new HMACSHA1(key));
        }

        public static string CalculateString(Stream src, string key) {
            return GenericHash.CalculateString(src, () => new HMACSHA1(Encoding.UTF8.GetBytes(key), true));
        }
