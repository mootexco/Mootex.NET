...
namespace Mootex.Hashing.SHA2 {

    public sealed class HMACSHA256Hash {

        public static byte[] CalculateBytes(Stream src, byte[] key) {
            return GenericHash.CalculateBytes(src, () => new HMACSHA256(key));
        }

        public static byte[] CalculateBytes(Stream src, string key) {
            return GenericHash.CalculateBytes(src, () => new HMACSHA256(Encoding.UTF8.GetBytes(key)));
        }
        ...
        public static string CalculateString(Stream src, byte[] key) {
            return GenericHash.CalculateString(src, () => new HMACSHA256(key));
        }

        public static string CalculateString(Stream src, string key) {
            return GenericHash.CalculateString(src, () => new HMACSHA256(Encoding.UTF8.GetBytes(key)));
        }
