...
namespace Mootex.Hashing {
    ...
    public sealed class HMACSHA384Hash {

        public static byte[] CalculateBytes(Stream src, byte[] key) {
            return GenericHash.CalculateBytes(src, () => new HMACSHA384(key));
        }

        public static byte[] CalculateBytes(Stream src, string key) {
            return GenericHash.CalculateBytes(src, () => new HMACSHA384(Encoding.UTF8.GetBytes(key)));
        }
        ...
        public static string CalculateString(Stream src, byte[] key) {
            return GenericHash.CalculateString(src, () => new HMACSHA384(key));
        }

        public static string CalculateString(Stream src, string key) {
            return GenericHash.CalculateString(src, () => new HMACSHA384(Encoding.UTF8.GetBytes(key)));
        }
